...
| 対象              | 現状の課題                                                            | 自動化による改善                                                                                | 実装方式（Phase 1実装済み）                                           |
| ----------------- | --------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------- | -------------------------------------------------------------------- |
| **①特許一覧作成** | ・手動で特許権者を検索<br/>・特許公報から手動転記<br/>・時間がかかる  | ・特許権者名入力で自動生成<br/>・J-PlatPat APIで自動取得<br/>・請求項を自動抽出・構造化         | ・**Phase 3で実装予定**<br/>・J-PlatPat連携                          |
| **②侵害調査**     | ・手動でプロンプト作成<br/>・ChatGPTへコピペ実行<br/>・結果を手動管理 | ・AIエージェントが自動分析<br/>・Web検索で製品仕様を自動収集<br/>・構成要件マッチングを自動判定 | ・**Phase 1で実装済み**<br/>・OpenAI Deep Research API<br/>・GitHub Actions Cron<br/>・Prisma + Supabase |
| **③売上推定**     | ・別途プロンプト作成<br/>・企業情報を手動調査<br/>・売上を手動推定    | ・侵害可能性○で自動起動<br/>・企業・財務情報を自動収集<br/>・収益ランク（A〜E）を自動判定       | ・**Phase 3で実装予定**<br/>・侵害額推定機能                          |
...
バックエンド
└─ Next.js API Routes
    ├─ /api/analyze/start - 分析開始
    ├─ /api/analyze/status - ステータス確認
    ├─ /api/analyze/result - 結果取得
    ├─ /api/patent-search/schedule - バッチスケジュール
    ├─ /api/cron/check-and-do - Cronハンドラー
    └─ /api/webhook/openai - Webhook受信

AI エンジン
└─ OpenAI Deep Research API
    ├─ o4-mini-deep-research-2025-06-26
    ├─ Web検索機能内蔵
    └─ 非同期処理 (background: true)

データストレージ
└─ Supabase PostgreSQL (Prisma経由)
    ├─ analysis_jobs テーブル
    ├─ ジョブステータス管理
    └─ 検索結果の永続化
...
:::info PoC開発計画
具体的なPoCフェーズの開発計画については、[PoC開発計画 v1.1](./poc-development-plan-v1.1.md)をご覧ください。

- **Phase 1（実装済み）**: 侵害調査の自動化
- **Phase 2（進行中）**: 業務利用可能性検証
- **Phase 3**: J-PlatPat連携、侵害額推定
- **Phase 4**: ログイン機能、利用料管理
:::
